...
Domain: digital-store-chi-teal.vercel.app
...
I wanted to build something closer to a real product than a tutorial project — a storefront with a management side for the people listing products, not just a static catalog, plus the social layer (reviews) that makes a store feel real. 

Tech stack
Framework: Next.js
Auth, database & storage: Firebase (Auth, Firestore) on the client; Firebase Admin SDK on the server for privileged operations
Language: TypeScript
Deployment: Vercel
